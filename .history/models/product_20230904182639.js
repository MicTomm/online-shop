...
class Product {
    ...
    async save() {
        ...
        if (this.id) {
            ...
            let productId = modelUtil.convertIntoObjectid(this.id);
            ...
            const pId = modelUtil.convertIntoObjectid(productId);
    ...
    static async fetchById(productId) {
        ...
        let id = modelUtil.convertIntoObjectid(productId);
    ...
    static async fetchMultipleProducts(cartProductIds) {
       
        const ids = cartProductIds.map(function (cartProductId) {

            return modelUtil.convertIntoObjectid(cartProductId);

        });

        //$in => find tutti gli id contenuti nell'array ids
        const productDocs = await db.getDb().collection('products').find({ _id: { $in: ids } }).toArray();

        return productDocs.map(function (productDoc) {
