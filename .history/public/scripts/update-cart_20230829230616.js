...
async function updateItemQuantity(action) {
    ...
    if (itemTotalPrice == 0) {
        updateCartItemForm.parentElement.previousElementSibling.children[1].textContent.remove();
    }

    const itemTotalPriceElement = updateCartItemForm.parentElement.previousElementSibling.children[1].textContent = itemTotalPrice;
    const cartTotalPriceParagraph = document.querySelector('#cart-total p');
    cartTotalPriceParagraph.textContent = cartTotalPrice; 
